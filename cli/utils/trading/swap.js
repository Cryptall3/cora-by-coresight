...
export async function getSwapQuote({
  ...
  return {
    ...
    estimatedOutput: attrs.output_amount?.quantity || attrs.estimation?.output_quantity?.float,
    outputMin: attrs.minimum_output_amount?.quantity || attrs.output_quantity_min?.float,
    gas: attrs.network_fee?.amount?.quantity || attrs.estimation?.gas,
    estimatedSeconds: attrs.estimated_time_seconds || attrs.estimation?.seconds,
    fee: {
      protocolPercent: attrs.protocol_fee?.percentage || attrs.fee?.protocol?.percent,
      protocolAmount: attrs.protocol_fee?.amount?.quantity || attrs.fee?.protocol?.quantity?.float,
    },
    liquiditySource: attrs.liquidity_source?.name,
    preconditions: attrs.preconditions_met || {},
    spender: attrs.transaction_approve?.evm?.to || attrs.asset_spender,
    transaction: attrs.transaction_swap?.evm || attrs.transaction_swap?.solana || attrs.transaction,
    fromChain,
    toChain: toChain || fromChain,
    slippageType: attrs.slippage_percent ? "custom" : "auto",
  };
}
